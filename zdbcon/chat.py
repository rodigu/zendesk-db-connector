...
class ZenChat(ZenAudit):
    ...
    def format_chat_history(event: dict, ticket_id: int) -> list[dict]:
        ...
        """
        return [{"ticket_id": ticket_id, "id": f"{event['id']}-{idx}", "content": h, "chat_id": event['id']} for idx, h in enumerate(event['value']['history']) if h['type']=="ChatMessage"]
    ...
    def get_chat_history(self, ticket_id: int) -> GeneratorExit:
        ...
        """
        return (h for audit in self.client.tickets.audits(ticket=ticket_id) for event in audit.events if event['type'] == 'ChatStartedEvent' for h in self.format_chat_history(event, ticket_id))
